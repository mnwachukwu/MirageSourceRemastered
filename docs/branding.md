...
All three are generated by [`tools/generate_app_icons.py`](../tools/generate_app_icons.py), which draws
them rather than rasterizing an SVG — the mark is nine rounded rectangles and the badges are a few
polygons, so the geometry lives in that file as numbers.

```sh
python tools/generate_app_icons.py     # needs Pillow: pip install pillow
```
