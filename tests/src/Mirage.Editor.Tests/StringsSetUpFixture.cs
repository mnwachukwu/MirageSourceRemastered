...
/// <summary>Assembly-wide one-time setup: loads <see cref="EditorStrings"/> ONCE, before any fixture in this
/// namespace runs, so a test whose code-under-test resolves a localized string works regardless of run order
/// or test filtering.
///
/// <para>In DEBUG, <c>EditorStrings.Get</c> THROWS on a missing/unloaded key (RELEASE returns a bracketed
...
/// namespace it is declared in (and descendants); all editor tests live in <c>Mirage.Editor.Tests</c>.</para></summary>
[SetUpFixture]
public sealed class StringsSetUpFixture
{
    [OneTimeSetUp]
    public void LoadStrings() => EditorStrings.Load(Path.Combine(AppContext.BaseDirectory, "lang"), "en");
}
