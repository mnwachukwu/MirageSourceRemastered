...
internal sealed class RecordingPersistence : IPersistenceService
{
    ...
    public Task<(SpellRecord[] records, int loaded)> LoadAllSpellsAsync() => Task.FromResult((Array.Empty<SpellRecord>(), 0));
    public Task<(ClassRecord[] records, int loaded)> LoadAllClassesAsync() => Task.FromResult((Array.Empty<ClassRecord>(), 0));
    public Task<(QuestRecord[] records, int loaded)> LoadAllQuestsAsync() => Task.FromResult((Array.Empty<QuestRecord>(), 0));
